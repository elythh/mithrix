...
![Build](https://github.com/meadow/nix-config/actions/workflows/ci.yaml/badge.svg)
...
```bash
 nix develop github:meadow/nix-config
```

Clone repository:

```bash
git clone https://github.com/meadow/nix-config.git ~/nix-config && cd ~/nix-config
```
